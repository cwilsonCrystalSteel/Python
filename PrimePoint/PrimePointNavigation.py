...
class PrimePointBase():
    ...
    def openReporting(self):
        ...
        self.ReportingButton = validateElement(self.driver, (By.LINK_TEXT, "Reporting"), 'reportingButton', True, True)
    ...
    def goToReportPage(self, ReportName):
        ...
        self.SearchBar = validateElement(self.driver, (By.XPATH, "//input[@placeholder='Search text']"), 'SearchBar',True, True)
        delete_range(self.SearchBar)
        self.SearchBar.send_keys(ReportName)
        
        self.Report = validateElement(self.driver, (By.XPATH, f"//div[contains(@class,'ag-cell-value') and contains(text(),'{ReportName}')]"), 'Report',True, True)
        self.Report.click()    
        
        self.RunReport = validateElement(self.driver, (By.XPATH, "//button[contains(@class,'btn') and contains(text(),'Run Report')]"), 'RunReport', True, True)
        self.RunReport.click()


    def downloadReportAsExcel(self):
        self.DownloadExcel = validateElement(self.driver, (By.XPATH, "//button[contains(@class,'btn') and contains(text(),'Download Report as Excel Spreadsheet')]"), 'DownloadExcel', True, True)
...
# move files
    ...
    def getCRYSSWSpectrumEmployerTaxExpenseJournalEntryExport(self, state, date):
        self.goToReportPage("CRYSSW Spectrum Employer Tax Expense Journal Entry Export")
        
        # search format  = "{date} - {state}..."
        
        self.PayrollSelection = validateElement(self.driver, (By.TAG_NAME, "NG-SELECT-XX"), 'PayrollSelection',True, True)
        self.PayrollSelection.click()
        self.PayrollSelectionSearchBar = validateElement(self.driver, (By.XPATH, "//input[@placeholder='Type to search...']"), 'PayrollSelectionSearchBar', True, True)
        ...
        # download
        self.downloadReportAsExcel()
        
        
    def findDownload(self):
        print('find the download')
        print('change the name to reflect the state & date ran')
        
                       
            
    def getLaborDistributionPayrollSummary(self):
        self.goToReportPage("Labor Distribution Payroll Summary")
...
x.tryLogin()
x.openReporting()
x.getCRYSSWSpectrumEmployerTaxExpenseJournalEntryExport(state='DE', date='2026-06-12')
