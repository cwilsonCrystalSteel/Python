...
def get_most_current_html(state='TN', start_date="03/06/2021", end_date="03/06/2021", download_folder="C:\\Users\\cwilson\\Downloads\\"):
    # from datetime import date, timedelta
    # today = date.today()
    # the_day = today - timedelta(days = 0)
    # date = the_day.strftime("%m/%d/%Y")
    print('Running TimeClock for: ' + start_date + " to " + end_date)
    ...
    while len(jobcodesummary) == 0:
        ...
        state_summary = "//*[contains(text(), '" + state + " Job Code Summary *')]"
        jobcodesummary = driver.find_elements_by_xpath(state_summary)
        print('Refreshed b/c jobcodesummary not found')
        # refreshes if it doesnt find the jobcodesummary

        
    print('Found JobCodeSummary')

    jobcodesummary[0].click()
