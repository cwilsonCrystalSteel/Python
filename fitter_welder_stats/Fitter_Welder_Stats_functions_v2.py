...
def return_sorted_and_ranked(df, ei, array_of_ids, col_name, defect_log, state, start_date, end_date, hours_types_pivot, void_entries_with_invalid_employee_number=True):
    ...
    df_na_ID = df[df[col_name].isna()].copy()
    df_na_ID['Name'] = 'Unknown'
    
    # now get rid of the rows for when the ID is invalid
    df = df[~df.index.isin(df_na_ID.index)]
    
    
    # if we cant match on the employee id to the ei
    df_na_ID2 = df.reset_index()
    df_na_ID2 = pd.merge(left=df_na_ID2, right=ei,
    ...
    df = df[~df.index.isin(df_na_ID2.index)]
    ...
    df_wrong_state = df.copy().reset_index()
    df_wrong_state = pd.merge(left=df_wrong_state, right=ei_state, 
                              left_on=col_name, right_on='ID',
                              how='left').set_index('index')
    
    df_wrong_state = df_wrong_state[df_wrong_state['Name'].isna()]
    df_wrong_state = df_wrong_state[df.columns]
    df_wrong_state2 = df_wrong_state.reset_index()
    # now figure out who the employee is
    df_wrong_state = pd.merge(left=df_wrong_state2, right=ei, 
                              left_on=col_name, right_on='ID',
                              how='left').set_index('index')
    ...
    df = df[~df.index.isin(df_wrong_state.index)]
    ...
    # now add the ei so that we can track it in the report
    df_notworked = pd.merge(left=df_notworked, right=ei,
                            left_on=col_name, right_on='ID',
                            how='left')
