...
for i in range(0,6):
    ...
    for state in states:
        ...
        try:
            ...
            starter = pd.read_excel(file_path)
    ...
    for start_dt in reversed_start_date_dict.keys():
        ...
        this_basis = get_information_for_clock_based_email_reports(this_start_date, this_end_date, exclude_terminated=False, ei=None) 
        ei = this_basis['Employee Information']
        basis = this_basis.copy()
        for i in range(1,(end_dt-start_dt).days):
            this_start_dt = start_dt + datetime.timedelta(days=i)
            this_end_dt = this_start_dt + datetime.timedelta(days = 1)
            this_start_date = this_start_dt.strftime("%m/%d/%Y")
            this_end_date = this_end_dt.strftime("%m/%d/%Y")
            this_basis = get_information_for_clock_based_email_reports(this_start_date, this_end_date, exclude_terminated=False, ei=ei) 
